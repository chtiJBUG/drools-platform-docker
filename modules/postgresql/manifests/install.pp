class postgresql::install ($tablespacedir, $platform, $guvnor, $guvnor_psswd, $platform_psswd) {
  ...
  file { ["${tablespacedir}/$guvnor", "${tablespacedir}/$platform"]: # create the directory
                                                                     # /var/lib/postgresql/9.1/main/myspace/guvnor
    ensure  => directory,
    owner   => 'postgres', # the owner of the directory will be postgres
...
#  exec { 'script-sql': # execute the scripts sql as postgres user
#    user    => 'postgres',
#    path    => '/usr/bin',
#    command => "psql -f /tmp/create_${guvnor}_user.sql;
#                psql -f /tmp/create_${platform}_user.sql;",
#    require => [
#      Service['postgresql'],
#      Package['postgresql-9.3'],
#      File["/tmp/create_${platform}_user.sql", "/tmp/create_${guvnor}_user.sql", "${tablespacedir}/$guvnor", "${tablespacedir}/$platform"
#        ]]
#  }
}
